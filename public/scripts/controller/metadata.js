...
myApp.controller('metadataCtrl', function ($scope, $state, $http, $stateParams, ngProgress, $window, Metadatas, _, Icon) {
    ...
    Metadatas.getMetadata({ Id: $scope.MetaId, state: $scope.CurrentPage }, function (metadata) {
        ...
        $scope.Celebrity = _.where(metadata.MasterList, { cm_name: "Celebrity" });
        $scope.Genres = _.where(metadata.MasterList, { cm_name: "Genres" });
        $scope.SubGenres = _.where(metadata.MasterList, { cm_name: "Sub Genres" });;
        $scope.Mood = _.where(metadata.MasterList, { cm_name: "Mood" });;
        $scope.Nudity = _.where(metadata.MasterList, { cm_name: "Nudity" });;
        $scope.Festival = _.where(metadata.MasterList, { cm_name: "Festival" });
        $scope.Religion = _.where(metadata.MasterList, { cm_name: "Religion" });;
        $scope.Languages = _.where(metadata.MasterList, { cm_name: "Languages" });
        ...
        $scope.RaagTaal = _.where(metadata.MasterList, { cm_name: "Raag Taal" });
        $scope.Instruments = _.where(metadata.MasterList, { cm_name: "Instruments" });
