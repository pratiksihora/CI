                ...
                mysql.getConnection('CMS', function (err, connection_ikon_cms) {
                    ...
                    async.parallel({
                        ...
                        VendorFiles: function (callback) {
                            //'SELECT parentid,cm_vendor,cm_content_type, CASE WHEN cm_state = 1    THEN cm_state WHEN cm_state = 2    THEN cm_state  WHEN cm_state = 6   THEN cm_state   WHEN cm_state = 5    THEN cm_state    WHEN cm_state =7   THEN cm_state WHEN propertyexpirydate < "' + currentdate + '"  THEN 6 WHEN propertyactive = 0   THEN 6  WHEN vd_end_on < "' + currentdate + '"   THEN 6 WHEN vd_is_active = 0   THEN 6  ELSE cm_state END AS cm_state  FROM  (select cm_vendor,cm_content_type,cm_property_id, CASE WHEN cm_state =5 THEN cm_state WHEN cm_state =7 THEN cm_state WHEN cm_expires_on < "' + currentdate + '" THEN 6 ELSE cm_state END AS cm_state from content_metadata WHERE cm_property_id is not null )cm inner join(SELECT cm_id as propertyid ,cm_expires_on as propertyexpirydate ,cm_is_active as propertyactive FROM content_metadata)prop on(cm.cm_property_id =prop.propertyid) inner join(SELECT vd_id ,vd_end_on  ,vd_is_active  FROM icn_vendor_detail)vd on(cm.cm_vendor =vd.vd_id) inner join (SELECT * FROM icn_manage_content_type)cnt on (cnt.mct_cnt_type_id = cm.cm_content_type) inner join (select cd_id as parentid,cd_name as parentname from catalogue_detail )parent on(parent.parentid  = cnt.mct_parent_cnt_type_id)' + vendorquery
