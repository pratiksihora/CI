                ...
                mysql.getConnection('CMS', function (err, connection_ikon_cms) {
                    ...
                    async.parallel({
                        ...
                        LyricsLanguages: function (callback) {
                            if (status == "edit") {
                                var query = connection_ikon_cms.query('select * from (SELECT * FROM content_metadata where cm_id =? )meta inner join(select * from multiselect_metadata_detail)mlm on(mlm.cmd_group_id = meta.cm_lyrics_languages) inner join(select * from catalogue_detail )cd on(cd.cd_id = mlm.cmd_entity_detail) inner join(select * from catalogue_master where cm_name in ("Languages"))cm on(cm.cm_id =cd.cd_cm_id)', [req.body.Id], function (err, LyricsLanguages) {
                        ...
                        MasterList: function (callback) {
                            var query = connection_ikon_cms.query('select * from ( select * from catalogue_detail)cd inner join(select * from catalogue_master where cm_name IN ("Lyricist","Song Type","Channel Distribution","Genres","Sub Genres","Mood","Languages","Celebrity","Festival","Religion","Raag Taal","Instruments","Nudity","Self Ranking","Search Keywords","Mode","InAppPurchase","Singers","Music Directors","Video Quality","Platform Supports") ) cm on (cd.cd_cm_id = cm.cm_id )', function (err, MasterList) {
            ...
            if (req.session.UserName) {
                mysql.getConnection('CMS', function (err, connection_ikon_cms) {
                    ...
                    function InsertUpdateMeta() {
                        ...
                        //add right
                            ...
                            function (val, callback) {
                                if (obj.cm_id) {
                                    var query = connection_ikon_cms.query('Update content_metadata set  cm_vendor = ? ,cm_content_type = ? ,cm_r_group_id = ? ,cm_title = ? ,cm_short_desc = ? ,cm_starts_from = ? ,cm_expires_on = ? , cm_property_id = ? ,  cm_display_title = ? ,  cm_celebrity = ? ,  cm_genre = ? ,  cm_sub_genre = ? ,   cm_protographer = ? ,  cm_mood = ? ,  cm_language = ? ,  cm_platform_support = ? ,  cm_nudity = ? ,  cm_parental_advisory = ? ,    cm_location = ? ,  cm_festival_occasion = ? ,  cm_religion = ? ,   cm_cp_content_id = ? , cm_content_duration = ? ,  cm_singer = ? , cm_music_director = ? , cm_music_director = ? , cm_content_quality = ? ,  cm_key_words = ? , cm_raag_tal = ? ,cm_instruments = ? , cm_long_description = ? ,  cm_mode = ? ,  cm_is_app_store_purchase = ? ,cm_rank = ? , cm_live_on = ? , cm_modified_on = ? , cm_modified_by = ? , cm_re_singer = ? ,cm_lyrics_languages = ? ,cm_lyricist = ? ,cm_song_type = ? where cm_id = ? ', [obj.cm_vendor, obj.cm_content_type, obj.cm_r_group_id, obj.cm_title, obj.cm_short_desc, new Date(obj.cm_starts_from), new Date(obj.cm_expires_on), obj.cm_property_id, obj.cm_title, obj.cm_celebrity, obj.cm_genre, obj.cm_sub_genre, obj.cm_protographer, obj.cm_mood, obj.cm_language, obj.cm_platform_support, obj.cm_nudity, obj.cm_parental_advisory, obj.cm_location, obj.cm_festival_occasion, obj.cm_religion, obj.cm_cp_content_id, obj.cm_content_duration, obj.cm_singer, obj.cm_music_director, obj.cm_music_director, obj.cm_content_quality, obj.cm_key_words, obj.cm_raag_tal, obj.cm_instruments, obj.cm_long_description, obj.cm_mode, obj.cm_is_app_store_purchase, obj.cm_rank, new Date(obj.cm_starts_from), new Date(), req.session.UserName, obj.cm_re_singer, obj.cm_lyrics_languages, obj.cm_lyricist, obj.cm_song_type, obj.cm_id], function (err, result) {
                                    ...
                                    var query = connection_ikon_cms.query('SELECT MAX(cm_id) as id FROM content_metadata', function (err, result) {
                                        if (err) {
                                            ...
                                            var metadata = {
                                                ...
                                                cm_release_year: obj.cm_release_year,
                ...
                , function (err, cm_id) {
                    if (err) {
                        ...
                        res.send({ success: true, message: "Metadata for Contenttype " + contenttype1 + (req.body.state.indexOf("add") > -1 ? " added" : " updated") + " successfully.", obj: obj, cm_id: obj.cm_id, contenttype: contenttype1 });
                    }
                });
                    }
