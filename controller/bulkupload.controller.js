                                                                            ...
                                                                            function (value, callback) {
                                                                                if (obj.cm_vendor && obj.cm_property_id && obj.r_rights_at_content_level && obj.r_rights_at_content_level != 0) {
                                                                                    if (obj.Country_Rights) {
                                                                                        ...
                                                                                        function loop(cnt) {
                                                                                            var cr = cnt;
                                                                                            var query = connection_ikon_cms.query('select * from ( select * from catalogue_master where cm_name in ("global_country_list"))cm inner join(select * from catalogue_detail where cd_name = ?) cd on (cd.cd_cm_id = cm.cm_id )', [country_rightsArray[cr].toLowerCase()], function (err, country_right_cd_id) {
